...
def evaluate(net, val_loader, loss, device):
    ...
    for X, Y in val_loader:
        Y = torch.round(Y * 10) - 1
        Y = F.one_hot(Y.to(torch.int64), 10).to(torch.float32)
        X, Y = X.to(device), Y.to(device)
        y_hat = net(X)
        
        l = loss(Y, y_hat.reshape(-1, 10))
        losses.append(l.mean().item())
        
        # predict = [x.item() for x in y_hat]
        # reality = [x.item() for x in Y]
        predict = torch.argmax(y_hat, dim=1).tolist()
        reality = torch.argmax(Y, dim=1).tolist()
...
def train(net, train_loader, val_loader, lr, num_epochs, device='cuda:0'):
    ...
    ce_loss_list: list[float] = []
    mse_loss_list: list[float] = []
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        for X, Y in train_loader:
            ...
            X, Y = X.to(device), Y.to(device)
            y_hat = net(X)
            ce_l = loss(Y, y_hat.reshape(-1, 10))
            
            # MSE
            y_hat_score = torch.sum(y_hat * torch.arange(1, 11, device=device) / 10, dim=1)
            mse_l = loss_mse(Y_score, y_hat_score)
            
            l = ce_l + mse_l*mse_scale
            # logger.debug(Y)
            ...
            ce_losses.append(ce_l.mean().item())
            mse_losses.append(mse_l.mean().item())
            losses.append(l.mean().item())
        ...
        ce_loss_list.append(sum(ce_losses) / len(ce_losses))
        mse_loss_list.append(sum(mse_losses) / len(mse_losses) * mse_scale)
        loss_list.append(sum(losses) / len(losses))
        val_loss_list.append(val_loss)
        plot_multi_loss([ce_loss_list, mse_loss_list, loss_list, val_loss_list], ['CE', 'MSE', 'Total', 'Val'], 'loss_img/loss.png')
...
def main():
    ...
    train(net, train_loader, test_loader, 0.00027, 500, device)
